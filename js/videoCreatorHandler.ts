...
const videoCreatorContainer = document.querySelector(
  "#video-creator-container"
) as HTMLDivElement;
const videoSendBtn = document.querySelector("#video-send-btn") as HTMLButtonElement;
const videoErrorMessage = document.querySelector("#video-error-message") as HTMLParagraphElement;
const videoSuccessMessage = document.querySelector("#video-success-message") as HTMLParagraphElement;
...
videoSendBtn.addEventListener("click", async () => {
  ...
  let newVideo = {};
  videoCreatorContainer.childNodes.forEach((child: any) => {
  ...
  try {
    ...
    const response = await fetch(
      `http://127.0.0.1:5000/video/${randomVideoId}`,
      {
        method: "PUT",
    ...
    videoErrorMessage.textContent = `ERROR: ${
